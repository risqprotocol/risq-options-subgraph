...
export class RisqOption extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id !== null, "Cannot save RisqOption entity without an ID");
    assert(
      id.kind == ValueKind.STRING,
      "Cannot save RisqOption entity with non-string ID. " +
        'Considering using .toHex() to convert the "id" to a string.'
    );
    store.set("RisqOption", id.toString(), this);
  }

  static load(id: string): RisqOption | null {
    return store.get("RisqOption", id) as RisqOption | null;
  }
