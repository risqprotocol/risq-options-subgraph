import { Create, Exercise, Expire, SetImpliedVolRateCall } from '../types/RisqPAXGOptions/RisqOptions'
import { Asset, RisqOption, ImpliedVolatility } from '../types/schema'
import { RisqOptions as Contract } from '../types/RisqPAXGOptions/RisqOptions'
import { Address } from '@graphprotocol/graph-ts';
import { BigIntOne, BigIntZero, getCreatePAXGPool, PAXG_OPTIONS_ADDR } from "../utils";

export function handleCreate(event: Create): void {
  let paxgOptions = Contract.bind(Address.fromString(PAXG_OPTIONS_ADDR));
  let liquidity_pool = getCreatePAXGPool()

  // Create option
  let option = new RisqOption("PAXG-" + event.params.id.toString());
  ...
  // Get from state 
  let option_data = paxgOptions.options(event.params.id)
...
export function handleExercise(event: Exercise): void {
  ...
  let option = RisqOption.load("PAXG-" + event.params.id.toString())
  if (option == null) {
    return
  }
...
export function handleSetImpliedVolRate(call: SetImpliedVolRateCall): void {
  ...
  let iv = new ImpliedVolatility("PAXG-" + liquidity_pool.numImpliedVolatility.toString())
