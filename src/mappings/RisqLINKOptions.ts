import { Create, Exercise, Expire, SetImpliedVolRateCall } from '../types/RisqLINKOptions/RisqOptions'
import { Asset, RisqOption, ImpliedVolatility } from '../types/schema'
import { RisqOptions as Contract } from '../types/RisqLINKOptions/RisqOptions'
import { Address } from '@graphprotocol/graph-ts';
import { BigIntOne, BigIntZero, getCreateLINKPool, LINK_OPTIONS_ADDR } from "../utils";

export function handleCreate(event: Create): void {
  let linkOptions = Contract.bind(Address.fromString(LINK_OPTIONS_ADDR));
  let liquidity_pool = getCreateLINKPool()

  // Create option
  let option = new RisqOption("LINK-" + event.params.id.toString());
  ...
  // Get from state 
  let option_data = linkOptions.options(event.params.id)
...
export function handleExercise(event: Exercise): void {
  ...
  let option = RisqOption.load("LINK-" + event.params.id.toString())
  if (option == null) {
    return
  }
...
export function handleSetImpliedVolRate(call: SetImpliedVolRateCall): void {
  ...
  let iv = new ImpliedVolatility("LINK-" + liquidity_pool.numImpliedVolatility.toString())
