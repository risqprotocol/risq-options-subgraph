...
} from "../types/LINKLiquidityPool/LiquidityPool";
import { 
  Provide,
  Withdraw,
  Profit,
...
import { BigIntOne, getCreateLINKPool } from "../utils";
...
export function handleProfit(event: ProfitEvent): void {
  ...
  profit.option = "LINK-" + event.params.id.toString()
...
export function handleLoss(event: LossEvent): void {
  ...
  loss.option = "LINK-" + event.params.id.toString()
