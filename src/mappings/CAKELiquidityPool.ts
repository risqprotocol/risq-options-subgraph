...
} from "../types/CAKELiquidityPool/LiquidityPool";
import { 
  Provide,
  Withdraw,
  Profit,
...
import { BigIntOne, getCreateCAKEPool } from "../utils";
...
export function handleProfit(event: ProfitEvent): void {
  ...
  profit.option = "CAKE-" + event.params.id.toString()
...
export function handleLoss(event: LossEvent): void {
  ...
  loss.option = "CAKE-" + event.params.id.toString()
