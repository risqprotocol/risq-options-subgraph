...
} from "../types/AMZNLiquidityPool/LiquidityPool";
import { 
  Provide,
  Withdraw,
  Profit,
...
import { BigIntOne, getCreateAMZNPool } from "../utils";
...
export function handleProfit(event: ProfitEvent): void {
  ...
  profit.option = "AMZN-" + event.params.id.toString()
...
export function handleLoss(event: LossEvent): void {
  ...
  loss.option = "AMZN-" + event.params.id.toString()
