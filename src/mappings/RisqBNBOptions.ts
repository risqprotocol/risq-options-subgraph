import { Create, Exercise, Expire, SetImpliedVolRateCall } from '../types/RisqBNBOptions/RisqOptions'
import { Asset, RisqOption, ImpliedVolatility } from '../types/schema'
import { RisqOptions as Contract } from '../types/RisqBNBOptions/RisqOptions'
import { Address, log } from '@graphprotocol/graph-ts';
import { BigIntOne, BigIntZero, BNB_OPTIONS_ADDR, getCreateBNBPool } from "../utils";

export function handleCreate(event: Create): void {
  let bnbOptions = Contract.bind(Address.fromString(BNB_OPTIONS_ADDR));
  let liquidity_pool = getCreateBNBPool()

  // Create option
  let option = new RisqOption("BNB-" + event.params.id.toString());
  ...
  // Get from state 
  let option_data = bnbOptions.options(event.params.id)
...
export function handleExercise(event: Exercise): void {
  ...
  let option = RisqOption.load("BNB-" + event.params.id.toString())
  if (option == null) {
    return
  }
...
export function handleSetImpliedVolRate(call: SetImpliedVolRateCall): void {
  ...
  let iv = new ImpliedVolatility("BNB-" + liquidity_pool.numImpliedVolatility.toString())
