import { Create, Exercise, Expire, SetImpliedVolRateCall } from '../types/RisqYFIOptions/RisqOptions'
import { Asset, RisqOption, ImpliedVolatility } from '../types/schema'
import { RisqOptions as Contract } from '../types/RisqYFIOptions/RisqOptions'
import { Address } from '@graphprotocol/graph-ts';
import { BigIntOne, BigIntZero, getCreateYFIPool, YFI_OPTIONS_ADDR } from "../utils";

export function handleCreate(event: Create): void {
  let yfiOptions = Contract.bind(Address.fromString(YFI_OPTIONS_ADDR));
  let liquidity_pool = getCreateYFIPool()

  // Create option
  let option = new RisqOption("YFI-" + event.params.id.toString());
  ...
  // Get from state 
  let option_data = yfiOptions.options(event.params.id)
...
export function handleExercise(event: Exercise): void {
  ...
  let option = RisqOption.load("YFI-" + event.params.id.toString())
  if (option == null) {
    return
  }
...
export function handleSetImpliedVolRate(call: SetImpliedVolRateCall): void {
  ...
  let iv = new ImpliedVolatility("YFI-" + liquidity_pool.numImpliedVolatility.toString())
