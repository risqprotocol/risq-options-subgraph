...
} from "../types/BCHLiquidityPool/LiquidityPool";
import { 
  Provide,
  Withdraw,
  Profit,
...
import { BigIntOne, getCreateBCHPool } from "../utils";
...
export function handleProfit(event: ProfitEvent): void {
  ...
  profit.option = "BCH-" + event.params.id.toString()
...
export function handleLoss(event: LossEvent): void {
  ...
  loss.option = "BCH-" + event.params.id.toString()
