import { Create, Exercise, Expire, SetImpliedVolRateCall } from '../types/RisqGOOGLOptions/RisqOptions'
import { Asset, RisqOption, ImpliedVolatility } from '../types/schema'
import { RisqOptions as Contract } from '../types/RisqGOOGLOptions/RisqOptions'
import { Address } from '@graphprotocol/graph-ts';
import { BigIntOne, BigIntZero, getCreateGOOGLPool, GOOGL_OPTIONS_ADDR } from "../utils";

export function handleCreate(event: Create): void {
  let googlOptions = Contract.bind(Address.fromString(GOOGL_OPTIONS_ADDR));
  let liquidity_pool = getCreateGOOGLPool()

  // Create option
  let option = new RisqOption("GOOGL-" + event.params.id.toString());
  ...
  // Get from state 
  let option_data = googlOptions.options(event.params.id)
...
export function handleExercise(event: Exercise): void {
  ...
  let option = RisqOption.load("GOOGL-" + event.params.id.toString())
  if (option == null) {
    return
  }
...
export function handleSetImpliedVolRate(call: SetImpliedVolRateCall): void {
  ...
  let iv = new ImpliedVolatility("GOOGL-" + liquidity_pool.numImpliedVolatility.toString())
