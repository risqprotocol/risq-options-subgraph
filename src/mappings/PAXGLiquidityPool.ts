...
} from "../types/PAXGLiquidityPool/LiquidityPool";
import { 
  Provide,
  Withdraw,
  Profit,
...
import { BigIntOne, getCreatePAXGPool } from "../utils";
...
export function handleProfit(event: ProfitEvent): void {
  ...
  profit.option = "PAXG-" + event.params.id.toString()
...
export function handleLoss(event: LossEvent): void {
  ...
  loss.option = "PAXG-" + event.params.id.toString()
