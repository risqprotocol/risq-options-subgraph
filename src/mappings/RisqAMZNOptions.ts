import { Create, Exercise, Expire, SetImpliedVolRateCall } from '../types/RisqAMZNOptions/RisqOptions'
import { Asset, RisqOption, ImpliedVolatility } from '../types/schema'
import { RisqOptions as Contract } from '../types/RisqAMZNOptions/RisqOptions'
import { Address } from '@graphprotocol/graph-ts';
import { BigIntOne, BigIntZero, getCreateAMZNPool, AMZN_OPTIONS_ADDR } from "../utils";

export function handleCreate(event: Create): void {
  let amznOptions = Contract.bind(Address.fromString(AMZN_OPTIONS_ADDR));
  let liquidity_pool = getCreateAMZNPool()

  // Create option
  let option = new RisqOption("AMZN-" + event.params.id.toString());
  ...
  // Get from state 
  let option_data = amznOptions.options(event.params.id)
...
export function handleExercise(event: Exercise): void {
  ...
  let option = RisqOption.load("AMZN-" + event.params.id.toString())
  if (option == null) {
    return
  }
...
export function handleSetImpliedVolRate(call: SetImpliedVolRateCall): void {
  ...
  let iv = new ImpliedVolatility("AMZN-" + liquidity_pool.numImpliedVolatility.toString())
