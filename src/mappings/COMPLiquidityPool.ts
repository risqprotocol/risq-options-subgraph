...
} from "../types/COMPLiquidityPool/LiquidityPool";
import { 
  Provide,
  Withdraw,
  Profit,
...
import { BigIntOne, getCreateCOMPPool } from "../utils";
...
export function handleProfit(event: ProfitEvent): void {
  ...
  profit.option = "COMP-" + event.params.id.toString()
...
export function handleLoss(event: LossEvent): void {
  ...
  loss.option = "COMP-" + event.params.id.toString()
