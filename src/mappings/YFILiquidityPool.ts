...
} from "../types/YFILiquidityPool/LiquidityPool";
import { 
  Provide,
  Withdraw,
  Profit,
...
import { BigIntOne, getCreateYFIPool } from "../utils";
...
export function handleProfit(event: ProfitEvent): void {
  ...
  profit.option = "YFI-" + event.params.id.toString()
...
export function handleLoss(event: LossEvent): void {
  ...
  loss.option = "YFI-" + event.params.id.toString()
