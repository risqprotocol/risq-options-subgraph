...
} from "../types/LTCLiquidityPool/LiquidityPool";
import { 
  Provide,
  Withdraw,
  Profit,
...
import { BigIntOne, getCreateLTCPool } from "../utils";
...
export function handleProfit(event: ProfitEvent): void {
  ...
  profit.option = "LTC-" + event.params.id.toString()
...
export function handleLoss(event: LossEvent): void {
  ...
  loss.option = "LTC-" + event.params.id.toString()
