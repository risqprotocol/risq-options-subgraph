...
} from "../types/GOOGLLiquidityPool/LiquidityPool";
import { 
  Provide,
  Withdraw,
  Profit,
...
import { BigIntOne, getCreateGOOGLPool } from "../utils";
...
export function handleProfit(event: ProfitEvent): void {
  ...
  profit.option = "GOOGL-" + event.params.id.toString()
...
export function handleLoss(event: LossEvent): void {
  ...
  loss.option = "GOOGL-" + event.params.id.toString()
