...
} from "../types/UNILiquidityPool/LiquidityPool";
import { 
  Provide,
  Withdraw,
  Profit,
...
import { BigIntOne, getCreateUNIPool } from "../utils";
...
export function handleProfit(event: ProfitEvent): void {
  ...
  profit.option = "UNI-" + event.params.id.toString()
...
export function handleLoss(event: LossEvent): void {
  ...
  loss.option = "UNI-" + event.params.id.toString()
