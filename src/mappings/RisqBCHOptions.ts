import { Create, Exercise, Expire, SetImpliedVolRateCall } from '../types/RisqBCHOptions/RisqOptions'
import { Asset, RisqOption, ImpliedVolatility } from '../types/schema'
import { RisqOptions as Contract } from '../types/RisqBCHOptions/RisqOptions'
import { Address } from '@graphprotocol/graph-ts';
import { BigIntOne, BigIntZero, getCreateBCHPool, BCH_OPTIONS_ADDR } from "../utils";

export function handleCreate(event: Create): void {
  let bchOptions = Contract.bind(Address.fromString(BCH_OPTIONS_ADDR));
  let liquidity_pool = getCreateBCHPool()

  // Create option
  let option = new RisqOption("BCH-" + event.params.id.toString());
  ...
  // Get from state 
  let option_data = bchOptions.options(event.params.id)
...
export function handleExercise(event: Exercise): void {
  ...
  let option = RisqOption.load("BCH-" + event.params.id.toString())
  if (option == null) {
    return
  }
...
export function handleSetImpliedVolRate(call: SetImpliedVolRateCall): void {
  ...
  let iv = new ImpliedVolatility("BCH-" + liquidity_pool.numImpliedVolatility.toString())
