import { Create, Exercise, Expire, SetImpliedVolRateCall } from '../types/RisqUNIOptions/RisqOptions'
import { Asset, RisqOption, ImpliedVolatility } from '../types/schema'
import { RisqOptions as Contract } from '../types/RisqUNIOptions/RisqOptions'
import { Address } from '@graphprotocol/graph-ts';
import { BigIntOne, BigIntZero, getCreateUNIPool, UNI_OPTIONS_ADDR } from "../utils";

export function handleCreate(event: Create): void {
  let uniOptions = Contract.bind(Address.fromString(UNI_OPTIONS_ADDR));
  let liquidity_pool = getCreateUNIPool()

  // Create option
  let option = new RisqOption("UNI-" + event.params.id.toString());
  ...
  // Get from state 
  let option_data = uniOptions.options(event.params.id)
...
export function handleExercise(event: Exercise): void {
  ...
  let option = RisqOption.load("UNI-" + event.params.id.toString())
  if (option == null) {
    return
  }
...
export function handleSetImpliedVolRate(call: SetImpliedVolRateCall): void {
  ...
  let iv = new ImpliedVolatility("UNI-" + liquidity_pool.numImpliedVolatility.toString())
