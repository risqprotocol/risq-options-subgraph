import { Create, Exercise, Expire, SetImpliedVolRateCall } from '../types/RisqCOMPOptions/RisqOptions'
import { Asset, RisqOption, ImpliedVolatility } from '../types/schema'
import { RisqOptions as Contract } from '../types/RisqCOMPOptions/RisqOptions'
import { Address } from '@graphprotocol/graph-ts';
import { BigIntOne, BigIntZero, getCreateCOMPPool, COMP_OPTIONS_ADDR } from "../utils";

export function handleCreate(event: Create): void {
  let compOptions = Contract.bind(Address.fromString(COMP_OPTIONS_ADDR));
  let liquidity_pool = getCreateCOMPPool()

  // Create option
  let option = new RisqOption("COMP-" + event.params.id.toString());
  ...
  // Get from state 
  let option_data = compOptions.options(event.params.id)
...
export function handleExercise(event: Exercise): void {
  ...
  let option = RisqOption.load("COMP-" + event.params.id.toString())
  if (option == null) {
    return
  }
...
export function handleSetImpliedVolRate(call: SetImpliedVolRateCall): void {
  ...
  let iv = new ImpliedVolatility("COMP-" + liquidity_pool.numImpliedVolatility.toString())
