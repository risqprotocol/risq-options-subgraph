...
} from "../types/TSLALiquidityPool/LiquidityPool";
import { 
  Provide,
  Withdraw,
  Profit,
...
import { BigIntOne, getCreateTSLAPool } from "../utils";
...
export function handleProfit(event: ProfitEvent): void {
  ...
  profit.option = "TSLA-" + event.params.id.toString()
...
export function handleLoss(event: LossEvent): void {
  ...
  loss.option = "TSLA-" + event.params.id.toString()
