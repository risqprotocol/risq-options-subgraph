...
} from "../types/BNBLiquidityPool/LiquidityPool";
import { 
  Provide,
  Withdraw,
  Profit,
...
import { BigIntOne, getCreateBNBPool } from "../utils";
...
export function handleProfit(event: ProfitEvent): void {
  ...
  profit.option = "BNB-" + event.params.id.toString()
...
export function handleLoss(event: LossEvent): void {
  ...
  loss.option = "BNB-" + event.params.id.toString()
