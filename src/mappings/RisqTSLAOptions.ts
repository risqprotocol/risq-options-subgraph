import { Create, Exercise, Expire, SetImpliedVolRateCall } from '../types/RisqTSLAOptions/RisqOptions'
import { Asset, RisqOption, ImpliedVolatility } from '../types/schema'
import { RisqOptions as Contract } from '../types/RisqTSLAOptions/RisqOptions'
import { Address } from '@graphprotocol/graph-ts';
import { BigIntOne, BigIntZero, getCreateTSLAPool, TSLA_OPTIONS_ADDR } from "../utils";

export function handleCreate(event: Create): void {
  let tslaOptions = Contract.bind(Address.fromString(TSLA_OPTIONS_ADDR));
  let liquidity_pool = getCreateTSLAPool()

  // Create option
  let option = new RisqOption("TSLA-" + event.params.id.toString());
  ...
  // Get from state 
  let option_data = tslaOptions.options(event.params.id)
...
export function handleExercise(event: Exercise): void {
  ...
  let option = RisqOption.load("TSLA-" + event.params.id.toString())
  if (option == null) {
    return
  }
...
export function handleSetImpliedVolRate(call: SetImpliedVolRateCall): void {
  ...
  let iv = new ImpliedVolatility("TSLA-" + liquidity_pool.numImpliedVolatility.toString())
