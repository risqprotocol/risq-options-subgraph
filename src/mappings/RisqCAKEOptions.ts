import { Create, Exercise, Expire, SetImpliedVolRateCall } from '../types/RisqCAKEOptions/RisqOptions'
import { Asset, RisqOption, ImpliedVolatility } from '../types/schema'
import { RisqOptions as Contract } from '../types/RisqCAKEOptions/RisqOptions'
import { Address } from '@graphprotocol/graph-ts';
import { BigIntOne, BigIntZero, getCreateCAKEPool, CAKE_OPTIONS_ADDR } from "../utils";

export function handleCreate(event: Create): void {
  let cakeOptions = Contract.bind(Address.fromString(CAKE_OPTIONS_ADDR));
  let liquidity_pool = getCreateCAKEPool()

  // Create option
  let option = new RisqOption("CAKE-" + event.params.id.toString());
  ...
  // Get from state 
  let option_data = cakeOptions.options(event.params.id)
...
export function handleExercise(event: Exercise): void {
  ...
  let option = RisqOption.load("CAKE-" + event.params.id.toString())
  if (option == null) {
    return
  }
...
export function handleSetImpliedVolRate(call: SetImpliedVolRateCall): void {
  ...
  let iv = new ImpliedVolatility("CAKE-" + liquidity_pool.numImpliedVolatility.toString())
